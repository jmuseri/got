package com.sa.bbva.got.gotApi.configuration;
...
public class SwaggerConfig extends WebMvcConfigurationSupport {
    ...
    public Docket parametriaApi() {
        return new Docket(DocumentationType.SWAGGER_2).groupName("Parametria").select()
                .apis(RequestHandlerSelectors.basePackage("com.sa.bbva.got.gotApi.controller.parametria")).paths(regex("/*.*"))
     ...
     "com.sa.bbva.got.gotApi.controller.funcional")).paths(regex("/*.*"))
     .build().apiInfo(metaData());}
    ...
    private ApiInfo metaData() {
        return new ApiInfoBuilder().title("GOT - Gestion Online de Tramites - API RESTful")
                .description("\"Gestion Online de Tramites Services \"").version("0.2.2")
